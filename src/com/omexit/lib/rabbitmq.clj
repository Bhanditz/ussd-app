      ...
      (try
        ...
        (let [{:keys [host port username password vhost]
              ...
              rabbitmq-conn-settings {:host host
                                      ...
                                      :password password
                                      :vhost vhost}]
             (log/debugf "rabbitMQ(%s)" rabbitmq-conn-settings)
...
(defn create-consumer-handler
      ...
      (fn [channel {:keys [delivery-tag] :as delivery-tag} ^bytes payload]
          (try
            (let [notification (json/read-str (String. payload "UTF-8") :key-fn keyword)]
                 (if (empty? notification)
                   (log/errorf "!invalidNotification -> %s" notification)
                   (do 
						(log/debugf "payload -> %s " notification)
